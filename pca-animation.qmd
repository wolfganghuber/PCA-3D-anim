...
view = function(
  ...
  for (theta in angle) {
    scatter3D(
      ...
      xlab = colnames(mat)[1], ylab = colnames(mat)[2], zlab = colnames(mat)[3],
    )
    for(a in seq_len(maxd))
      lines3D(
        x = pca$center[1] + pca$scale[1] * pca$rotation[1, a] * 2 * c(-1,1), 
        y = pca$center[2] + pca$scale[2] * pca$rotation[2, a] * 2 * c(-1,1),
        z = pca$center[3] + pca$scale[3] * pca$rotation[3, a] * 2 * c(-1,1),
        col = c("orange", "red")[a], lwd = 2, add = TRUE
      )
    ## See the exaples of scatter3D: droplines till the fitted surface
  }
}
```
