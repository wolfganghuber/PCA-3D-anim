  ...
  for (i in seq_along(theta)) {
    ...
    if (is.null(surf)) {
      ...
      for(a in seq_len(num_axes)) {
        lines3D(
          x = pca$rotation[1, a] * ax_len * c(-1,1), 
          y = pca$rotation[2, a] * ax_len * c(-1,1),
          z = pca$rotation[3, a] * ax_len * c(-1,1),
          col = "#606060", lwd = 2, add = TRUE
        ) # lines3D
        text3D(
          x = abs(pca$rotation[1, a]) * ax_len, 
          y = abs(pca$rotation[2, a]) * ax_len,
          z = abs(pca$rotation[3, a]) * ax_len,
          labels = paste0("PC", a),
          col = "#000000", add = TRUE
        ) # lines3D
      } # for a
...
normalvec = pca$rotation[, 3] / sqrt(sum(pca$rotation^2))
theta2 = asin(normalvec[3])
phi2   = atan2(normalvec[2], normalvec[1]) 
...
#| label: saveGIF
saveGIF({ 
  view(theta = seq(-180, 180, length.out = 120), phi = 15)
  view(theta = seq(-180, 180, length.out = 120), phi = 15, num_axes = 1)
  view(theta = seq(-180, 180, length.out = 120), phi = 15, num_axes = 2)
  view(theta = seq(-180, 180, length.out = 120), phi = 15, surf = theplane)
  view(theta = seq(-180, theta2, length.out = 120), 
       phi   = seq(15, phi2, length.out = 120), surf = theplane)
  }, 
